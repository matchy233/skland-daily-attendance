...
# 可选：隐藏角色名（设置任意值即可启用；false/0/off/no 关闭）
...
因为每次计划任务执行不一定签到成功，所以本服务在除了 GitHub Actions 的情况下使用时，会通过每天执行的计划任务（默认 8:00 和 20:00）保证当天的签到任务成功，所以需要持久化存储对应日期的签到状态避免成功后重复签到。

> [!WARNING]
> 项目默认使用每天 8:00 和 20:00 的计划任务执行签到任务，如需调整执行频率，需要在 `nitro.config.ts` 中手动修改。
